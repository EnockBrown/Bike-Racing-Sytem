...
<body>
		...
		<div class = "panel panel-default">
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "table table-bordered">
					...
					<tbody>
						<?php
							$query = $conn->query("SELECT * FROM `transaction` NATURAL JOIN `guest` NATURAL JOIN `bike` WHERE `status` = 'Check In'") or die(mysqli_query());
							while($fetch = $query->fetch_array()){
						?>
						<tr>
							<td><?php echo $fetch['firstname']." ".$fetch['lastname']?></td>
							<td><?php echo $fetch['bike_type']?></td>
							<td><?php echo $fetch['bike_no']?></td>
							...
							<td><?php if($fetch['extra_days'] == "0"){ echo "None";}else{echo $fetch['extra_days'];}?></td>
	...
	<div style = "text-align:right; margin-right:10px;" class = "navbar navbar-default navbar-fixed-bottom">
		<label>&copy; Copyright 2019 </label>
	</div>
</body>
